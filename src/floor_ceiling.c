...
/*   Updated: 2025/03/31 17:12:11 by rafaria          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int find_floor_ceiling(t_struct *map, char **map_table, char *letter)
{
	...
	while (map_table[i])
	{
        if (ft_strnstr(map_table[i], letter, ft_strlen_size_t(map_table[i])) != 0)
		{
...
int verify_rgb(t_struct *map, char *map_string, char *letter)
{
	...
	{
		printf("value=%s\n", split_result[j]);
		if (ft_isalnum_inferior_255(split_result[j]) == -1 || count_characters(map_string) == -1)
		{
