...
/*   Updated: 2025/04/14 17:45:25 by aneumann         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_move_forward(t_ray *ray)
{
	...
	if (ray->mapp.map_dis[(int)(ray->posY + ray->dirY * MOVE_SPEED)][(int)ray->posX] != '1')
		ray->posY += ray->dirY * MOVE_SPEED;
	if (ray->mapp.map_dis[(int)ray->posY][(int)(ray->posX + ray->dirX * MOVE_SPEED)] !='1')
...
void	ft_move_left(t_ray *ray)
{
	...
	if (ray->mapp.map_dis[(int)ray->posY][(int)(ray->posX - ray->dirY * MOVE_SPEED)]!= '1')
		ray->posX += ray->dirY * MOVE_SPEED;
	if (ray->mapp.map_dis[(int)(ray->posY + ray->dirX * MOVE_SPEED)][(int)ray->posX]!= '1')
